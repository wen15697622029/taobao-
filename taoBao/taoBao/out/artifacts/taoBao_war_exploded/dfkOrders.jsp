...
<c:forEach items="${sessionScope.orders}" var="order" varStatus="loop">
                <tr >
                    ...
                    <td><input type="button" value="详情" onclick="dd(${loop.count-1})"></td>
                        <form method="post" action="pay" onsubmit="return check(${sessionScope.user.money},${order.money})">
                    <td><input type="hidden" value="${order.id}" name="ordersId">
                            <input type="submit" value="付款" >
                    </td>
                        </form>
                        <form method="post" action="deleteOrders">
                    <td>
                        <input type="hidden" value="${order.id}" name="ordersId">
        ...
        <div id="d32" >
            <a href="dfkOrders?currentPage=${sessionScope.currentPage-1==0?sessionScope.currentPage:sessionScope.currentPage-1}">上一页</a>
            <a href="dfkOrders?currentPage=${sessionScope.currentPage}">第${sessionScope.currentPage}页</a>
            共${sessionScope.totalPages}页
            <a href="dfkOrders?currentPage=${sessionScope.currentPage+1>sessionScope.totalPages?sessionScope.currentPage:sessionScope.currentPage+1}">下一页</a>
            <form action="dfkOrders" method="post">
